(module conjure.client.shen.stdio
...
(def- cfg (config.get-in-fn [:client :shen :stdio]))
...
(defn on-filetype []
  (mapping.buf :n :ShenStart (cfg [:mapping :start]) *module-name* :start)
  (mapping.buf :n :ShenStop (cfg [:mapping :stop]) *module-name* :stop))
